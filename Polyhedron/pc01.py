...
def __range__(left, right, inclusive):
    ...
    let
    end = not inclusive ? right: ascending ? right + 1: right - 1
...
//= == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == ==
const
color_hex_palette = ["#ff7777", "#dddddd", "#889999", "#fff0e5",
               "#aa3333", "#ff0000", "#ffffff", "#aaaaaa"
                     ]
let
PALETTE = color_hex_palette;  # GLOBAL
const
palette =
...
def hsl2rgb(h, s, l):
    ...
    def(rgb):
        let
        r_hex = int(parseInt(255 * rgb[0], 10)).toString(16)
        let
        g_hex = int(parseInt(255 * rgb[1], 10)).toString(16)
        let
        b_hex = int(parseInt(255 * rgb[2], 10)).toString(16)
    ...
    def():
        ...
        for let i = 0; i < 100; i++:
            ...
            let
            rgb = hsl2rgb(h, s, l)
            newpalette.append(floatstohex(rgb))
    ...
    def(poly):
        ...
        for var f of poly.faces:
            ...
            elif COLOR_METHOD is "signature":
            ...
            poly.face_classes.append(clr)

        print(_.toArray(colormemory).length + " face classes")
        return poly
    ...
    const
    zsortIndex = _.zip(centroids, normals, __range__(0, poly.faces.length, False))
    //.sort(planesort)
    .sort(zcentroidsort)
    .map(x= > x[2])
    ...
    centersArray.append(mult(1.0 / face.length, fcenter))
    ...
    normalsArray = []
    for let face of this.faces:
        normalsArray.append(normal(face.map(vidx= > this.vertices[vidx])))
    ...
    data(:
    const
    nEdges = (this.faces.length + this.vertices.length) - 2;  # E = V + F - 2
    return `${this.faces.length}
    faces, ${nEdges}
    edges, ${this.vertices.length}
    vertices
    `
    ...
    return `min
    edge
    length ${this.minEdgeLength().toPrecision(2)} < br > ` +
    `min
    face
    radius ${this.minFaceRadius().toPrecision(2)}
    `
...
# square of edge length
    const
d2 = mag2(sub(this.vertices[e[0]], this.vertices[e[1]]))
if d2 < min2:
    min2 = d2
...
# Check distance from center to each edge.
    const
de2 = linePointDist2(this.vertices[e[0]], this.vertices[e[1]], c)
if de2 < min2:
    min2 = de2
...
for f of this.faces:
    const
norm = normal(f.map(v= > this.vertices[v]))
...
poly.vertices.append([-cos(i * theta), -sin(i * theta), -h])

for i = 0; i < n; i++) {  # vertex #'s n to 2n-1 around other
poly.vertices.append([-cos(i * theta), -sin(i * theta), h])
}

poly.faces.append(__range__(n - 1, 0, True)); // top
poly.faces.append(__range__(n, 2 * n, False)); // bottom
...
def(n):
    ...
    poly.vertices.append([r * cos(i * theta), r * sin(i * theta), h])

}
for i = 0; i < n; i++) {  # vertex #'s n...2n-1 around other
poly.vertices.append([r * cos((i + 0.5) * theta), r * sin((i + 0.5) * theta), -h])
}

poly.faces.append(__range__(n - 1, 0, True)); // top
poly.faces.append(__range__(n, (2 * n) - 1, True)); // bottom
...
def(n):
    ...
    for i = 0; i < n; i++) {  # vertex #'s 0...n-1 around one face
    poly.vertices.append([-cos(i * theta), -sin(i * theta), -0.2])

}
poly.vertices.append([0, 0, height]);  # apex

poly.faces.append(__range__(n - 1, 0, True));  # base
for i = 0;
i < n;
i + +) {  # n triangular sides
...
def(n, alpha, height):
    ...
    poly.faces.append(__range__(2 * n - 1, 0, True));  # base
    poly.faces.append(__range__(2 * n, 3 * n - 1, True));  # top
