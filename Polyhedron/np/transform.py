from np.polyhedron import polyflag, polyhedron
from np.geo import *
...
class transform():
    ...
    def gyro(poly: polyhedron) -> polyhedron:
        ...
        for i, f in enumerate(poly.faces):
            flag.newV(f'center{i}', unit(poly.centers[i]))
        for i, f in enumerate(poly.faces):
            v1, v2 = f[-2:]
    ...
    def quinto(poly: polyhedron) -> polyhedron:
        ...
        for i, f in enumerate(poly.faces):
            centroid = calc_centroid(poly.vertices[f])
            v1, v2 = f[-2:]
            for v3 in f:
...
if __name__ == '__main__':
    from np.seeds import tetrahedron
