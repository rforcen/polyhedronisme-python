...
const cross = (d1, d2) => [(d1[1] * d2[2]) - (d1[2] * d2[1]),
  (d1[2] * d2[0]) - (d1[0] * d2[2]),
  (d1[0] * d2[1]) - (d1[1] * d2[0])
];
...
const tween = (vec1, vec2, t) => [((1 - t) * vec1[0]) + (t * vec2[0]),
  ((1 - t) * vec1[1]) + (t * vec2[1]),
  ((1 - t) * vec1[2]) + (t * vec2[2])
];
...
const insetN = function (poly, n, inset_dist, popout_dist) {
  ...
  if (inset_dist === undefined) {
    inset_dist = 0.5;
  }
  if (popout_dist === undefined) {
    popout_dist = -0.2;
  }
